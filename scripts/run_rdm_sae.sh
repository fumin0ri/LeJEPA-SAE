...
output_dir="${1:-${OUTPUT_DIR:-runs/rdm-sae/d${feature_dim}-rho${rho}-${activation}-slope${slope}-rec${reconstruction_weight}-rdm${rdm_weight}-scale${target_scale}-seed${seed}-steps${steps}}}"
if [[ ! -f "$config" ]]; then
...
exec lejepa-train --config "$config" \
  ...
  --set "loss.axis_weight=${AXIS_WEIGHT:-1.0}" \

...
output_dir="${1:-${OUTPUT_DIR:-runs/rdm-sae/d${feature_dim}-rho${rho}-${activation}-slope${slope}-rec${reconstruction_weight}-rdm${rdm_weight}-scale${target_scale}-wp${wasserstein_power}-axis${axis_weight}-seed${seed}-steps${steps}}}"
if [[ ! -f "$config" ]]; then
